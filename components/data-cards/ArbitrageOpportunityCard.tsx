    ...
    <div className="group relative w-full rounded-2xl overflow-hidden bg-[var(--bg-surface)] border border-[var(--border-subtle)] hover:border-[var(--border-hover)] hover:shadow-[var(--shadow-glow)] transition-all duration-300">
      ...
      <div className="relative px-4 pt-4 pb-4 flex flex-col gap-3">
        ...
        <div className="text-center py-2">
          ...
          <p className="text-5xl font-black tabular-nums text-emerald-300 leading-none">
            +{edge.toFixed(2)}%
          </p>
          {opp.sport && (
          ...
          <div className="flex items-stretch gap-2">
            {sides.map((s, i) => {
              ...
              return (
                ...
                >
                  <div className={`text-[9px] font-bold uppercase tracking-widest mb-1 ${isLeg1 ? 'text-emerald-500' : 'text-teal-400'}`}>
                    Leg {i + 1}
                  </div>
                  <div className="text-[11px] font-bold text-white/80 truncate mb-1.5">{s.name}</div>
                  <div className={`text-xl font-black tabular-nums ${isLeg1 ? 'text-emerald-300' : 'text-teal-300'}`}>
        ...
        {onAsk && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/25 hover:bg-emerald-500/20 hover:border-emerald-500/50 text-[12px] font-bold text-emerald-300 hover:text-emerald-200 transition-all duration-200"
          >
