import { NextRequest, NextResponse } from 'next/server';
import {
  requireAuth,
  AuthRequiredError,
  unauthorized,
  badRequest,
  parseJsonBody,
  JsonParseError,
  internalError,
} from '@/lib/api/route-helpers';

const VALID_ALERT_TYPES = [
  'odds_change',
  'line_movement',
  'player_prop',
  'arbitrage',
  'kalshi_price',
  'game_start',
] as const;

/**
 * GET /api/alerts
 * Returns the authenticated user's alerts, newest first.
 */
export async function GET() {
  try {
    const { supabase, user } = await requireAuth();

    const { data, error } = await supabase
      .from('user_alerts')
      .select('*')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false })
      .limit(50);

    if (error) {
      console.error('[Alerts] GET failed:', error);
      return internalError(error.message);
    }

    return NextResponse.json({ success: true, data: data ?? [] });
  } catch (err) {
    if (err instanceof AuthRequiredError) return unauthorized();
    console.error('[Alerts] GET error:', err);
    return internalError();
  }
}

/**
 * POST /api/alerts
 * Creates a new alert for the authenticated user.
 */
export async function POST(req: NextRequest) {
  try {
    const { supabase, user } = await requireAuth();

    const body = await parseJsonBody<{
      title?: string;
      alert_type?: string;
      sport?: string;
      team?: string;
      player?: string;
      description?: string;
      threshold?: number | null;
      max_triggers?: number | null;
    }>(req);

    const { title, alert_type, sport, team, player, description, threshold, max_triggers } = body;

    if (!title || typeof title !== 'string' || !title.trim()) {
      return badRequest('Title is required');
    }
    if (!alert_type || !VALID_ALERT_TYPES.includes(alert_type as typeof VALID_ALERT_TYPES[number])) {
      return badRequest('Invalid alert_type');
    }

    const STR_MAX = 100;
    const DESC_MAX = 500;
    if (
      title.trim().length > STR_MAX ||
      (sport && String(sport).length > STR_MAX) ||
      (team && String(team).length > STR_MAX) ||
      (player && String(player).length > STR_MAX) ||
      (description && String(description).length > DESC_MAX)
    ) {
      return badRequest('Field value too long');
    }

    const { data, error } = await supabase
      .from('user_alerts')
      .insert({
        user_id: user.id,
        title: title.trim().slice(0, STR_MAX),
        alert_type,
        sport: sport ? String(sport).slice(0, STR_MAX) : null,
        team: team ? String(team).slice(0, STR_MAX) : null,
        player: player ? String(player).slice(0, STR_MAX) : null,
        description: description ? String(description).slice(0, DESC_MAX) : null,
        threshold: threshold ?? null,
        max_triggers: max_triggers ?? 1,
        condition: { marketType: alert_type, threshold: threshold ?? null },
        is_active: true,
        trigger_count: 0,
      })
      .select()
      .single();

    if (error) {
      console.error('[Alerts] POST failed:', error);
      return internalError(error.message);
    }

    return NextResponse.json({ success: true, data }, { status: 201 });
  } catch (err) {
    if (err instanceof AuthRequiredError) return unauthorized();
    if (err instanceof JsonParseError) return badRequest(err.message);
    console.error('[Alerts] POST error:', err);
    return internalError();
  }
}
