...
export function EmptyState({
  ...
  return (
    <div className={`ml-11 ${className}`}>
      ...
      <div className="relative bg-gradient-to-br from-[var(--bg-overlay)] to-[var(--bg-overlay)] backdrop-blur-xl rounded-2xl p-8 border border-[var(--border-subtle)] shadow-xl overflow-hidden">
        ...
        <div className="relative space-y-6">
              ...
              {isOffseason ? (
                <div className="space-y-1">
                  ...
                  {seasonInfo!.nextGameEstimate && (
                    <p className="text-xs text-[var(--text-faint)]">{seasonInfo!.nextGameEstimate}</p>
                  )}
                </div>
              ) : (
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="px-4 text-xs font-semibold text-[var(--text-faint)] bg-[var(--bg-overlay)] uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {suggestions.map((suggestion, index) => {
              ...
              return (
                <button
                  key={index}
                  className="group relative flex items-start gap-3 p-4 rounded-xl bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] border border-[var(--border-subtle)] hover:border-[var(--border-hover)] transition-all duration-300 text-left"
                  onClick={() => {
                    console.log('[v0] Empty state suggestion clicked:', suggestion.text);
                  }}
                >
                  <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-[var(--bg-surface)]/40 to-[var(--bg-elevated)]/40 border border-[var(--border-hover)]/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <Icon className="w-5 h-5 text-[var(--text-muted)] group-hover:text-blue-400 transition-colors" />
                  </div>
                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="text-xs font-semibold text-[var(--text-faint)] uppercase tracking-wider mb-1">
                      {suggestion.category}
                    </div>
                    <div className="text-sm font-medium text-foreground/80 group-hover:text-white transition-colors leading-snug">
                      {suggestion.text}
                    </div>
                  </div>
                </button>
