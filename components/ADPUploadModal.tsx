  ...
  return (
    ...
    >
      ...
      <div className="bg-black/20 rounded-lg p-3 mb-3">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Step 1 — Download the TSV</p>
        <a
          href={downloadUrl}
          target="_blank"
        ...
        <p className="text-[10px] text-gray-500 mt-1">
          On the page, click the <span className="text-gray-300 font-semibold">Download</span> button to get the TSV file.
        </p>
      </div>

      {/* Step 2 — Upload area */}
      <div className="bg-black/20 rounded-lg p-3 mb-3">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Step 2 — Upload the file</p>
        ...
        {status === 'idle' && (
          ...
          >
            ...
            <span className="text-xs text-gray-300">Click to select TSV file, or drag &amp; drop here</span>
            <span className="text-[10px] text-gray-500">.tsv or .csv accepted</span>
          </button>
        )}
        ...
        {status === 'uploading' && (
          <div className="flex items-center gap-3 p-3">
            ...
            <span className="text-xs text-gray-300">Parsing and importing players…</span>
          </div>
        )}
        ...
        {status === 'error' && (
          <div className="flex items-start gap-2.5 p-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-0.5 break-words">{message}</p>
              <button
                onClick={() => { setStatus('idle'); if (fileRef.current) fileRef.current.value = ''; }}
                className="text-[10px] text-violet-400 hover:text-violet-300 mt-1 underline"
              >
