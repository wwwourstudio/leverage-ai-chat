...
export function DatabaseStatusBanner({ onDismiss }: DatabaseStatusBannerProps) {
  ...
  useEffect(() => {
    ...
    async function checkDatabaseStatus() {
      try {
        const response = await fetch('/api/insights').catch(() => null);
        ...
        if (!response || !response.ok) {
          ...
          setTimeout(() => { if (!cancelled) setDismissed(true); }, 2000);
          return;
        }
        ...
        if (data.setupRequired) {
          ...
          setMessage(String(data.message ?? 'Database tables need to be created'));
        } else if (data.success) {
          setStatus('connected');
          setMessage('Database connected and ready');
          setTimeout(() => { if (!cancelled) setDismissed(true); }, 3000);
        } else {
          setStatus('error');
          setMessage(String(data.message ?? 'Unable to connect to database'));
        }
      } catch {
        if (cancelled) return;
        setStatus('connected');
