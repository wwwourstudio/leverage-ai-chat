...
**Example 1: Fetching and Displaying Live Odds**

```typescript
// In your component
import { fetchDynamicCards } from '@/lib/data-service';

async function loadBettingCards() {
  const cards = await fetchDynamicCards({
    sport: 'NBA',
  ...
  cards.forEach(card => {
    console.log(`${card.title}: ${card.data.matchup}`);
    console.log(`Best line: ${card.data.bestLine}`);
    console.log(`Edge: ${card.data.edge}`);
  });
}
```

**Example 2: Custom AI Analysis with Context**

```typescript
// Send contextual analysis request
const response = await fetch('/api/analyze', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    message: "Compare the Lakers spread value across all books",
    context: {
      sport: 'NBA',
      analysisType: 'sports-betting',
      team: 'Lakers',
...
console.log('Confidence:', data.trustMetrics.finalConfidence);
```

**Example 3: Monitoring User Insights**

```typescript
// Fetch user insights periodically
async function getUserInsights() {
  const response = await fetch('/api/insights');
  const { insights } = await response.json();
  
  return {
    portfolioValue: insights.totalValue,
    roi: insights.roi,
    winRate: insights.winRate,
    totalInvested: insights.totalInvested,
    performance: insights.roi > 0 ? 'positive' : 'negative'
  };
}

// Update UI with insights
const insights = await getUserInsights();
console.log(`Portfolio: $${insights.portfolioValue}`);
console.log(`ROI: ${insights.roi}%`);
console.log(`Win Rate: ${insights.winRate}%`);
```

**Example 4: Custom Configuration Management**

```typescript
// Fetch and use dynamic configuration
...
async function initializeApp() {
  const config = await getConfigs([
    ...
    { key: 'max_cards_per_request', defaultValue: 10, category: 'ui' }
  ]);
  
  // Use configuration
  const portfolio = config.default_invested_amount;
  const threshold = config.high_confidence_threshold;
  
  console.log(`Initialized with portfolio: $${portfolio}`);
  console.log(`High confidence threshold: ${threshold}`);
}
```
...
Run this comprehensive diagnostic to verify all systems:

```bash
#!/bin/bash
# save as: scripts/health-check.sh

echo "=== NFC Assistant Health Check ==="
...
curl -s "https://api.the-odds-api.com/v4/sports?apiKey=$ODDS_API_KEY" | \
  jq -r 'if . then "✓ Odds API responding" else "✗ Odds API failed" end'

# 4. Test database connection
echo "\n4. Database Test:"
curl -s https://your-app.com/api/insights | \
  jq -r 'if .success then "✓ Database connected" else "✗ Database error" end'

# 5. Generate test cards
echo "\n5. Card Generation Test:"
curl -s -X POST https://your-app.com/api/cards \
  -H "Content-Type: application/json" \
  -d '{"sport":"NBA","limit":3}' | \
  jq -r '.cards | length as $count | "Generated \($count) cards"'

echo "\n=== Health Check Complete ==="
```

### Debugging Dynamic Cards Generation

If cards aren't generating, follow this diagnostic workflow:

**Step 1: Verify Frontend Request**
```javascript
// In browser console
console.log('[DEBUG] Fetching cards...');
const response = await fetch('/api/cards', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ sport: 'NBA', limit: 5 })
});
const data = await response.json();
console.log('[DEBUG] Response:', data);
console.log('[DEBUG] Cards received:', data.cards?.length || 0);
```
...
[API] Extracted parameters: sport=NBA, limit=5
[API] Odds API Key configured: YES
[API] Processing 15 live odds events
[API] Generated 3 cards from live odds data
```

**Step 3: Verify Odds Data**
```bash
# Test odds API directly
curl -s "https://api.the-odds-api.com/v4/sports/basketball_nba/odds?apiKey=$ODDS_API_KEY&regions=us" | jq '.[0]'
```

**Step 4: Check Time Filtering**
```javascript
// Check if events are in valid time range
const events = await fetch('/api/odds?sport=basketball_nba').then(r => r.json());
events.forEach(event => {
  const gameTime = new Date(event.commence_time);
  const hoursUntil = (gameTime - new Date()) / (1000 * 60 * 60);
  console.log(`${event.home_team} vs ${event.away_team}: ${hoursUntil.toFixed(1)}h`);
});
```

**Step 5: Validate Card Transformation**
```javascript
// Check if transformation is working
const response = await fetch('/api/cards', {
  method: 'POST',
  body: JSON.stringify({ sport: 'NBA', limit: 10 })
});
const data = await response.json();

console.log('Card types:', data.cards.map(c => c.type));
console.log('Real data flags:', data.cards.map(c => c.realData));
```
...
```javascript
// Test AI with verbose logging
const testAnalysis = async () => {
  console.log('[TEST] Sending AI request...');
  
  const response = await fetch('/api/analyze', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: 'Analyze Lakers spread',
      context: { sport: 'NBA' }
    })
  });
  ...
  console.log('[TEST] Response received:', {
    ...
    hasText: data.analysis?.length > 0,
    trustScore: data.trustMetrics?.finalConfidence,
    model: data.model,
...
if (error) {
  ...
  console.log('✓ Database connected, sample:', data);
}
```
