...
CREATE TABLE IF NOT EXISTS api.adp_upload_history (
...
CREATE INDEX IF NOT EXISTS idx_adp_upload_history_sport ON api.adp_upload_history (sport, is_active, uploaded_at DESC);
...
CREATE POLICY IF NOT EXISTS "adp_upload_history_read" ON api.adp_upload_history FOR SELECT USING (true);

-- Only service_role can insert/update (server-side API routes)
CREATE POLICY IF NOT EXISTS "adp_upload_history_service_write" ON api.adp_upload_history FOR ALL TO service_role USING (true) WITH CHECK (true);
