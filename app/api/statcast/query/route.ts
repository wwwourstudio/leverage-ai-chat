...
export async function GET(request: NextRequest) {
  ...
  if (batter) filters.batter = parseInt(batter, 10);

  const pitcher = searchParams.get('pitcher');
  if (pitcher) filters.pitcher = parseInt(pitcher, 10);
  ...
  if (pitchType) filters.pitch_type = pitchType.toUpperCase();
  ...
  if (error) {
    return NextResponse.json({ error }, { status: error.includes('not yet migrated') ? 503 : 500 });
  }
