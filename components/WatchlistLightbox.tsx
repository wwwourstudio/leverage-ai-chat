...
function PlayerRow({ entry, onRemove }: { entry: WatchlistEntry; onRemove: (name: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/3 transition-colors group">
      {/* Avatar */}
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-white leading-tight truncate">{entry.name}</p>
        <div className="flex items-center gap-1.5 mt-0.5">
          {entry.team && (
            <span className="text-[10px] font-semibold text-gray-500">{entry.team}</span>
          )}
          {entry.team && <span className="text-gray-700 text-[10px]">·</span>}
          <span className="text-[10px] font-bold text-blue-400/80 uppercase">{entry.position}</span>
          <span className="text-gray-700 text-[10px]">·</span>
          <span className="text-[10px] text-gray-600">Added {addedDate}</span>
        </div>
      </div>
      ...
      <button
        ...
        className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-500/15 text-gray-600 hover:text-rose-400 transition-all duration-150"
...
export function WatchlistLightbox({ isOpen, onClose }: WatchlistLightboxProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-sm bg-[oklch(0.09_0.012_280)] border-l border-white/10 flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-black text-white">Player Watchlist</h2>
              <p className="text-[10px] text-gray-500">{entries.length} {entries.length === 1 ? 'player' : 'players'} saved</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/8 rounded-xl text-gray-400 hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center">
              ...
              <div>
                <p className="text-sm font-bold text-gray-400">No players saved yet</p>
        ...
        {entries.length > 0 && (
          <div className="px-4 py-3 border-t border-white/10">
            <button
              ...
              className="w-full py-2 rounded-xl text-[11px] font-bold text-gray-500 hover:text-rose-400 hover:bg-rose-500/8 border border-transparent hover:border-rose-500/20 transition-all"
            >
              Clear all players
            </button>
          </div>
