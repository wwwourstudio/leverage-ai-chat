...
export async function loadADPFromSupabase(_sport = 'mlb', _allowStale = false): Promise<NFBCPlayer[] | null> {
  // This file is imported by client components, so we cannot use Supabase here
  // The server components and API routes use adp-data.server.ts for actual DB access
  return null;
}
